...
const MENU = [
  ...
  { name:'Tofu Laksa',        desc:'Creamy coconut laksa broth with silken tofu, rice noodles, crunchy tofu puffs, and sambal.',         price:'RM 13.90', badge:'Signature',  badgeColor:'bg-cyan-100 text-cyan-700',    emoji:'🍜' },
  { name:'Tau Fu Fah',        desc:'Traditional silken tofu pudding served warm or chilled with ginger syrup. The perfect sweet ending.',  price:'RM 5.90',  badge:'Dessert',    badgeColor:'bg-purple-100 text-purple-700',emoji:'🍮' },
]
...
export default function Home() {
  ...
  // Floating soybean canvas — fixed full-page background
  useEffect(() => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      beans.push({
        ...
        swayFreq: Math.random() * 0.018 + 0.008,
  ...
  return (
    <>
      ...
      {/* Fixed full-page floating soybean canvas */}
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-[#2c1a0e]">
            {['Menu','About','Testimonials','Contact'].map(s => (
              <a key={s} href={`#${s.toLowerCase()}`} className="nav-link hover:text-[#d4891a] transition-colors">{s}</a>
            ))}
            <a href={waLink} target="_blank" rel="noopener noreferrer"
              className="btn-dark px-5 py-2 rounded-full text-sm font-semibold">Order Now</a>
          </div>
          <button onClick={() => setMenuOpen(!menuOpen)}
            className="md:hidden p-2 rounded-lg text-[#2c1a0e] hover:bg-amber-100 transition-colors" aria-label="Toggle menu">
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#fef9ec] border-t border-amber-100 px-6 py-4 flex flex-col gap-4 text-sm font-medium text-[#2c1a0e]">
            {['Menu','About','Testimonials','Contact'].map(s => (
              <a key={s} href={`#${s.toLowerCase()}`} onClick={() => setMenuOpen(false)} className="hover:text-[#d4891a] transition-colors">{s}</a>
            ))}
            <a href={waLink} target="_blank" rel="noopener noreferrer" onClick={() => setMenuOpen(false)}
              className="btn-dark text-center px-5 py-2.5 rounded-full font-semibold">Order via WhatsApp</a>
          </div>
        )}
      </nav>
      ...
      <main id="top" className="relative z-10">
          ...
          <div className="relative max-w-3xl mx-auto text-center" style={{zIndex:2}}>
            ...
            <div className="animate-fade-up mb-5" style={{animationDelay:'0.2s'}}>
              <span className="inline-flex items-center gap-2 bg-[#00b4d8]/10 border border-[#00b4d8]/25 text-[#007fa0] text-xs font-bold tracking-widest uppercase px-5 py-2 rounded-full">
                🫘 EST. {BIZ.established} &nbsp;·&nbsp; Nilai, Negeri Sembilan
              </span>
            </div>

            <p className="animate-fade-up text-xl md:text-2xl text-[#5c3317] font-light mb-3 tracking-wide"
              style={{animationDelay:'0.3s',fontFamily:'Playfair Display, serif'}}>
              {BIZ.tagline}
            </p>
            ...
            <div className="animate-fade-up flex flex-col sm:flex-row gap-4 justify-center" style={{animationDelay:'0.5s'}}>
              <a href="#menu" className="btn-dark px-9 py-4 rounded-full font-semibold text-base">
                Explore Our Menu
              </a>
              <a href={waLink} target="_blank" rel="noopener noreferrer"
        ...
        <div className="craft-divider py-3 flex flex-wrap items-center justify-center gap-x-8 gap-y-1 overflow-hidden">
          {['🫘 TOFU HAUZ','🍚 FRESH DAILY','💚 PLANT-BASED','🏠 HOMEMADE','✦ EST. 2018','🍜 NILAI, NS'].map((t,i) => (
            <span key={i} className="text-[#ffd97a] text-xs font-bold tracking-widest uppercase whitespace-nowrap">{t}</span>
          ))}
        </div>
        ...
        <section id="menu" className="py-24 px-6 relative" style={{background:'#fef9ec'}}>
          ...
          <div className="max-w-6xl mx-auto relative z-10">
            <div className="text-center mb-16">
              <p className="text-[#00b4d8] text-xs font-bold tracking-widest uppercase mb-3">What We Serve</p>
              <h2 className="text-4xl md:text-5xl font-bold text-[#2c1a0e]">Our Menu</h2>
              <div className="section-divider" />
              <p className="text-[#7a4a28] mt-6 max-w-xl mx-auto">Every dish is crafted fresh with the finest tofu and seasonal ingredients. Simple food, serious flavour.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {MENU.map((item) => (
                <div key={item.name} className="warm-card rounded-2xl p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <a href={`${waLink}&text=Hi%20Tofu%20Hauz!%20I%27d%20like%20to%20order%20the%20${encodeURIComponent(item.name)}`}
                      ...
                      className="btn-dark text-xs font-bold px-4 py-2 rounded-full">Order</a>
                  </div>
                </div>
              ))}
            </div>
            <div className="text-center mt-14">
              <p className="text-[#7a4a28] text-sm mb-4">Can&apos;t decide? Let us recommend!</p>
              <a href={waLink} target="_blank" rel="noopener noreferrer"
        ...
        <section id="about" className="py-24 px-6 relative" style={{background:'#fff4e4'}}>
          <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            ...
            <div>
              <p className="text-[#00b4d8] text-xs font-bold tracking-widest uppercase mb-3">Our Story</p>
              <h2 className="text-4xl font-bold text-[#2c1a0e] mb-6">Why We Started Tofu Hauz</h2>
              <div className="section-divider" style={{margin:'0 0 24px'}} />
              <p className="text-[#7a4a28] leading-relaxed mb-4">
              ...
              <p className="text-[#7a4a28] leading-relaxed mb-6">
                Every bowl, skewer, and dessert we serve is a testament to what tofu can really do when it&apos;s treated with care, creativity, and good spices. Made fresh, made with heart, right from our home kitchen since {BIZ.established}.
              </p>
        ...
        <section id="testimonials" className="py-24 px-6 relative" style={{background:'#fef9ec'}}>
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-16">
              <p className="text-[#00b4d8] text-xs font-bold tracking-widest uppercase mb-3">Happy Customers</p>
              <h2 className="text-4xl md:text-5xl font-bold text-[#2c1a0e]">What People Say</h2>
              <div className="section-divider" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {TESTIMONIALS.map((t) => (
                <div key={t.name} className="testimonial-card warm-card rounded-2xl p-8">
                  <div className="flex mb-4">{Array.from({length:t.rating}).map((_,i) => <span key={i} className="text-amber-400 text-lg">★</span>)}</div>
                  <p className="text-[#5c3317] leading-relaxed mb-6 pt-4">{t.text}</p>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm"
                      style={{background:'linear-gradient(135deg,#d4891a,#00b4d8)'}}>
                      {t.name.charAt(0)}
                    </div>
                    <span className="font-semibold text-[#2c1a0e]">{t.name}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="py-20 px-6 text-center relative overflow-hidden" style={{background:'#2c1a0e'}}>
          ...
          <div className="max-w-2xl mx-auto relative z-10">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{fontFamily:'Playfair Display, serif'}}>Ready to Order?</h2>
            <p className="text-amber-200 mb-10 leading-relaxed">Skip the queue — order ahead via WhatsApp. Fresh, homemade, ready for pickup or delivery.</p>
            <a href={waLink} target="_blank" rel="noopener noreferrer"
        ...
        <section id="contact" className="py-24 px-6 relative" style={{background:'#fff8f0'}}>
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-16">
              <p className="text-[#00b4d8] text-xs font-bold tracking-widest uppercase mb-3">Come Visit Us</p>
              <h2 className="text-4xl md:text-5xl font-bold text-[#2c1a0e]">Find Us</h2>
              <div className="section-divider" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
        ...
        <footer className="py-10 px-6 text-center text-sm border-t border-amber-900/20" style={{background:'#1a0f05'}}>
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-amber-700/70 mb-1">{BIZ.tagline}</p>
            <p className="text-amber-900/50">© {new Date().getFullYear()} {BIZ.name}. All rights reserved.</p>
          </div>
        </footer>
